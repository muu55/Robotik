...
int turninit(int start, int val){ // start equals current compass value // val equals angle to turn for
  ...
  printf("Turninit function complete!");
}
...
if (status == 'l'){ // do we have to go left?
  printf("status is l");
  blink(10);
  turn(wanted); // turn the bot left
  if ((cmps12.kompass - 20 < wanted) && (cmps12.kompass + 20 < wanted) || (cmps12.kompass - 20 > wanted) && (cmps12.kompass + 20 > wanted)){status = 'n';}
  cmps12_update();
  startwert = cmps12.kompass; // set new correct start value
  continue;}
  else{continue;}

if ((farbe == 'r') && (status == 'n')) {blink(10); printf("RED DETECTED");} // blink if color red is detected
else if (farbe == 'b'){ // fuck is that black?
	status = 'b'; // go reverse!
  printf("BLACK DETECTED");
}
else if ((dist(0) >= 150.0) && (status == 'n')){status = 'l'; continue;}
else if ((dist(1) >= 150.0) && (status == 'n')){
  motor_on(1000, MOTOR1_FWD|MOTOR2_FWD);
  usleep(1000000);
}
else if ((dist(1) <= 150.0) && (dist(0) >= 150.0) && (status == 'n')){status = 'l';}
