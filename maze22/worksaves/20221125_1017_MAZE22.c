...
int wanted = turninit(cmps12.kompass, +900);
...
int wand = 0;
if (dist(0) < 150.0) wand += 100;
if (dist(1) < 150.0) wand +=  10;
if (dist(2) < 150.0) wand +=   1;
...
if (status == 'b'){ // do we have to drive reverse?
  motor_on(1000, MOTOR1_REV|MOTOR2_REV);
  usleep(1000000); // wait for longer motor count
  status = 'r'; // after drive right
}
else if (status == 'r'){ // do we have to go right?
  cmps12_update();
  turn(wanted); // turn the bot right
  if ((cmps12.kompass - 20 < wanted) && (cmps12.kompass + 20 < wanted) || (cmps12.kompass - 20 > wanted) && (cmps12.kompass + 20 > wanted)){
    status = 'n'; // if turning is complete set status do nothing to do
    startwert = cmps12.kompass; // set new correct start value
    onetime = true; // let him do the onetime thing next time he has to turn
    printf("Turning complete");
    continue;}
  else{continue;}
}
else if (status == 'l'){ // do we have to go left?
cmps12_update();
  turn(wanted); // turn the bot left
  if ((cmps12.kompass - 20 < wanted) && (cmps12.kompass + 20 < wanted) || (cmps12.kompass - 20 > wanted) && (cmps12.kompass + 20 > wanted)){
    status = 'n'; // if turning is complete set status do nothing to do
    startwert = cmps12.kompass; // set new correct start value
    onetime = true; // let him do the onetime thing next time he has to turn
...
else if ((farbe == 'r') && (status == 'n')) { // if color red detected and nothing else to do
  //blink(10); // blink if color red is detected
  printf("Blink red");
  } 
else if (farbe == 'b'){ // fuck is that black?
	status = 'b'; // go reverse!
}
else if ((dist(2) >= 150.0) && (status == 'n')) { // if right is clear and nothing else to do
  motor_on(10, MOTOR1_FWD|MOTOR2_FWD); // drive straight
  status = 'r'; // set status to drive right
  continue; // get out
  } 
else if ((dist(1) >= 150.0) && (status == 'n')) { // if nothing in front and nothing else to do
  motor_on(10, MOTOR1_FWD|MOTOR2_FWD); // drive straight
}
else if ((dist(1) <= 150.0) && (dist(0) >= 150.0) && (status == 'n')) { // if the front is blocked and the left is free and nothing else to do
  status = 'l'; // set status to drive left
  }
...
if (dist(1) < 100.0){motor_off();}
if (dist(1) < 50.0){motor_on(100, MOTOR1_REV|MOTOR2_REV);} 
